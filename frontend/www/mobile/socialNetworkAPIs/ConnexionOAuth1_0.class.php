...
abstract class ConnexionOAuth1_0 extends Connexion
{
	...
	private /*void*/ function getRequestToken(/*string*/ $callbackUrl)// throws CUrlException, HttpException
	{
		...
		$response	= $this->httpSignedGet(static::getRequestTokenUrl(), array(), $params);
	...
	public /*void*/ function button()
	{
		...
		if($query_string != "")
			$query_string = '&'.$query_string;
?><a rel="external" href="?connexion=<?=urlencode(static::getSocialNetworkName())?><?=htmlspecialchars($query_string)?>" class="<?=preg_replace('#[^a-z0-9]#','',strtolower(static::getSocialNetworkName()))?>"><span><?= static::getSocialNetworkName()?></span></a><?php
	}
	/**
