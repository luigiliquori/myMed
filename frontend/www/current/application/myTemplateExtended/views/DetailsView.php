	...
	<div data-role="content" >
		...
		<div data-role="collapsible-set" data-theme="c" data-content-theme="d">
			...
			<h3><?= _("Description") ?></h3>
				...
				<!-- Reputation -->
				<?php  // Only user wit myTemplateExtended extended profile can rate 
									...
									<div class="ui-block-b">
										...
										<div data-role="controlgroup" data-type="horizontal" style="float: right;">
											<? if($item->accepted!='accepted'): ?>
												<a style="float:left;" type="button" href="#" onclick='generate_accept_popup("<?= $item->publisher ?>","<?= $item->pred1 ?>","<?= $item->pred2 ?>","<?= $item->pred3 ?>","<?= $item->author ?>","<?= $this->result->maxappliers ?>","<?= $this->result->currentappliers ?>","<?= $this->result->area ?>","<?= $this->result->category ?>","<?= $this->result->locality ?>","<?= $this->result->organization ?>","<?= $this->result->end ?>","<?= str_replace('"',"&#39;", $this->result->text)t ?>","<?= $item->title ?>");' data-theme="g" data-inline="true" data-mini="true"><?= _('Accept') ?></a>
											<? endif; ?>
