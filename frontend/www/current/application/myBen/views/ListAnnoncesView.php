	...
	<div data-role="content">
		...
		<? if (!isset($this->extendedProfile)) : ?>
			<div class="mm-welcome"><p>
                ...
                <?= _("le service de mise en relation des bénévoles et des associations, dans Nice et sa région via <strong>Nice Bénévolat</strong>.") ?><br/>
				<br/>
				        <?= _("	Vous êtes une association ? Vous vous recherchez des bénévoles ?") ?> <br/>
						<a 
		...
		<? if (sizeof($this->annonces) == 0) : ?>
			...
			<a  
				...
				<?= _("Afficher toutes les annonces") ?>
			</a>
		<? else : ?>
				...
				<li>
					<a 	data-ajax="false"
						...
						<p>
							...
							<b><?= _("Parution") ?>:</b> <?= $annonce->begin ?><br/>
							<? if (!empty($annonce->end)) : ?>
								<b><?= _("Fin") ?>:</b> <?= $annonce->end ?>
								<br/>
							<? endif ?>
						</p>
					</a>
