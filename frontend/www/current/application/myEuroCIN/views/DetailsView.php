			 ...
			 <? if(isset($_SESSION['user']) && !$_SESSION['user']->is_guest) :?>
				...
				<? if($this->result->validated!="waiting"): ?>
					...
					<div style="position: absolute; right: 150px; padding-top:40px;">
						...
						<a href="javascript:postToFeed('<?= 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']?>','<?= $this->result->getTitle() ?>', '<?= str_replace('"',"&#39;", $this->result->text) ?>', '<?= str_replace("MYMED_", "", $this->result->publisherID) ?>', '<?= APPLICATION_NAME ?>')"><img src="img/facebookShare.png"/></a>	
					</div>
