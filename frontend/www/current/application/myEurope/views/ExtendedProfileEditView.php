...
<div data-role="content">
	<form action="?action=ExtendedProfile" method="post" name="ExtendedProfileForm" id="ExtendedProfileForm" data-ajax="false" class="compact">
		...
		<fieldset id="textinputu0" data-role="controlgroup" data-mini="true" data-type="horizontal" style="display:inline-block;vertical-align: middle;">
			...
			<input type="radio" name="role" id="radio-view-a" value="Association" <?= $_SESSION['myEuropeProfile']->role == "Association"?"checked='checked'":"" ?>/>
			<label for="radio-view-a"><?= _('Association') ?></label>
			<input type="radio" name="role" id="radio-view-b" value="Entreprise" <?= $_SESSION['myEuropeProfile']->role == "Entreprise"?"checked='checked'":"" ?>/>
			<label for="radio-view-b"><?= _('Company') ?></label>
			<input type="radio" name="role" id="radio-view-e" value="EtabPublic" <?= $_SESSION['myEuropeProfile']->role == "EtabPublic"?"checked='checked'":"" ?>/>
			<label for="radio-view-e"><?= _('State-owned enterprise') ?></label>
			<input type="radio" name="role" id="radio-view-f" value="Mairie" <?= $_SESSION['myEuropeProfile']->role == "Mairie"?"checked='checked'":"" ?>/>
			<label for="radio-view-f"><?= _('Town hall') ?></label>
			<input type="radio" name="role" id="radio-view-g" value="Région" <?= $_SESSION['myEuropeProfile']->role == "Région"?"checked='checked'":"" ?>/>
			<label for="radio-view-g"><?= _('Region') ?></label>
			<input type="radio" name="role" id="radio-view-h" value="Département" <?= $_SESSION['myEuropeProfile']->role == "Département"?"checked='checked'":"" ?>/>
			<label for="radio-view-h"><?= _('Department') ?></label>
			<input type="radio" name="role" id="radio-view-i" value="Autre" <?= $_SESSION['myEuropeProfile']->role == "Autre"?"checked='checked'":"" ?>/>
