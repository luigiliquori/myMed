	...
	switch(node.nodeType)
	{
		case document.ELEMENT_NODE:
			...
			if(allChildren && node.childNodes && node.childNodes.length > 0)
				...
				{
					if( (newNode.tagName.toLowerCase() == "style")&&(!newNode.type) )
						newNode.setAttribute("type", "text/css");
					if( (newNode.tagName.toLowerCase() == "style")&&(node.text)&&(newNode.styleSheet !== undefined) ) 
						newNode.styleSheet.cssText = node.text;
					else
