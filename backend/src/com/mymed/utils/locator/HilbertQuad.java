...
public class HilbertQuad {
  ...
  static protected HilbertQuad decode(final long key) throws IllegalArgumentException {
    ...
    final HilbertQuad quad = new HilbertQuad(key, maxLevel, latitudeRange[0], longitudeRange[0], latitudeRange[1],
        longitudeRange[1], QuadType.A);
    // The first bit chooses the initial longitude range. Either (-180,0) or
    // (0,180)
  ...
  /*
   * Returns the quad of level @param i that contains the point @param lat,
   * 
   * @param lon.
   */
  ...
  /*
   * Get the sub-quad of @param quad at the position @param pos. 0 is the
   * bottom-left sub-quad 1 is the bottom-right sub-quad 2 is the top-left
