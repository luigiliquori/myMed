...
public final class MConverter {
   ...
   * @throws InternalBackEndException
   *           if one of the strings is null, or when a wrong encoding is used
   */
   ...
   * <p>
   * No assumptions are made on the value stored in the ByteBuffer, it has to be
   * an integer
   * 
   * @param buffer
  ...
  public static String convertStreamToString(final InputStream is, final int length) throws InternalBackEndException {
    try {
      if (length > 0) {
        ...
        byteBuff.compact();
        return MConverter.byteBufferToString(byteBuff);
      } else {
        final BufferedReader buffRead = new BufferedReader(new InputStreamReader(is, Charset.forName("UTF-8")));
