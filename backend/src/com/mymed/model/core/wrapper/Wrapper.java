...
public class Wrapper {
		...
		try {
			...
			if (social_network.equals("myMed")) {
				...
				String password = new String(node.getSimpleColumn("Mymed",
						...
						consistencyOnRead), "UTF8");
				return new User(id, name, gender, locale, updated_time,
						profile, profile_picture, social_network, email,
						password);
			} else {
				return new User(id, name, gender, locale, updated_time,
						profile, profile_picture, social_network);
			}
		} catch (Exception e) {
	...
	public void setProfile(User user) {
		...
		try {
			...
			if (user.getSocial_network().equals("myMed")) {
				// USER EMAIL
				node.setSimpleColumn("Mymed", columnFamily, user.getId(),
						"email".getBytes("UTF8"), user.getEmail().getBytes(
								"UTF8"), consistencyOnWrite);
				// USER PASSWORD
				node.setSimpleColumn("Mymed", columnFamily, user.getId(),
						"password".getBytes("UTF8"), user.getPassword()
								.getBytes("UTF8"), consistencyOnWrite);
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
	...
	public byte[] selectColumn(String tableName, String primaryKey,
		...
		return node.getSimpleColumn(keysapce, columnFamily, primaryKey, columnName
				.getBytes("UTF8"), consistencyOnRead);
	}
