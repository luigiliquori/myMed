...
public class VerdictManager {
  ...
  private final DbTableAdapter adapter;
  private final ReputationAlgorithm reputationAlgorithm;
  private final ReputationUpdaterFactory reputationUpdaterFactory;

  public VerdictManager(final CassandraWrapper w) {
  ...
  /**
   * This method causes the update of a reputation of a given user (the charged)
   * caused by the submission of a new feedback by an other user (the judge)
   * 
   * @param idApp
   *          the id of the used application
   * @param idJudge
   *          the id of the user playing the role of judge
   * @param isJudgeProducer
   *          a flag telling us whether the judge is a producer of a consumer
   * @param idCharged
   *          the id of the user playing the role of charged
   * @param vote
   *          the provided feedback
   * @return true if the update operation exits with success, false otherwise
   */
  public boolean updateReputation(final String idApp, final String idJudge, final boolean isJudgeProducer,
      final String idCharged, final double vote) {
    // starting a new transaction
    adapter.createTransaction();

    // creating a new verdict with accociated data
    final Verdict v = new Verdict();
    ...
    try {
      // checking if the user being the judge and the user being the charged
      ...
      final Thread t = new Thread(
          reputationUpdaterFactory.createNewReputationUpdater(idCharged, idApp, isJudgeProducer));
      t.start();
   ...
   */
  public String createAggregation(final String id) {

    final String idAggregation = id + String.valueOf(System.currentTimeMillis());

    final VerdictAggregation va = new VerdictAggregation();
  ...
  /**
   * This method deletes an aggregation of judgments (note that it does not
   * delete the judgments being part of the aggregation)
   * 
   * @param idAggregation
   *          the id of the aggregation to be deleted
   */
  public boolean deleteAggregation(final String idAggregation) {
    try {
      adapter.createTransaction();
  ...
  /**
   * This methods creates a new judgment given by a judge about a given charged
   * and adds this judgment to an already created aggregation
   * 
   * @param idAggregation
   *          the id of aggregation where the new judgment will be added
   * @param idApp
   *          the id of used application
   * @param idJudge
   *          the id of the user being the judge
   * @param isJudgeProducer
   *          a flag telling us whether the judge is a producer or a consumer
   * @param idCharged
   *          the id of the used being the charged
   * @param vote
   *          the provided feedback
   * @return true if the update operation exits with success, false otherwise
   */
  public boolean updateAggregation(final String idAggregation, final String idApp, final String idJudge,
      final boolean isJudgeProducer, final String idCharged, final double vote) {
    // starting a new transaction
    adapter.createTransaction();
    ...
    try {
      ...
      final VerdictAggregation va = adapter.getRecordVerdictAggregationById(idAggregation);
      ...
      // the following method updates the VerdictAggregationList and the
      // AuxOrderVerdictList tables
      adapter.persistVerdictToAggregation(va, v);

      adapter.commit(); // FIX remove this commit

      adapter.createTransaction();
      final double newScore = reputationAlgorithm.computeReputation(idAggregation, adapter);
  ...
  //
  // MReputationBean readReputation = reputationManager.read("6utent2", "app1",
  // false);
  // System.out.println("Reputazione iniziale u2: " +
  // readReputation.getReputation());
  // System.out.println("numero di giudizi: " +
  // readReputation.getNoOfRatings());
  //
  //
  // verdictManager.updateReputation("app1", "utente1", true, "6utent2", 0.7);
  // System.out.println("Reputazione dopo il primo voto: " +
  // reputationManager.read("6utent2", "app1", false).getReputation());
  // verdictManager.updateReputation("app1", "utente3", true, "6utent2", 0.5);
  //
  // System.out.println("Reputazione dopo il secondo voto: " +
  // reputationManager.read("6utent2", "app1", false).getReputation());
  //
  //
  ...
  // verdictManager.updateAggregation(aggid1, "app1", "utente1", true,
  // "6utent2", 0.4);
  // System.out.println("Aggregation score dopo il primo verdetto: " +
  // aggregationManager.read(aggid1).getReputation());
  // verdictManager.updateAggregation(aggid1, "app1", "utente2", true,
  // "6utent2", 0.6);
  // System.out.println("Aggregation score dopo il secondo verdetto: " +
  // aggregationManager.read(aggid1).getReputation());
  //
  // verdictManager.updateAggregation(aggid1, "app1", "utente2", true,
  // "utente3", 0.5);
  ...
  // verdictManager.updateAggregation(aggid2, "app2", "utente1", true,
  // "utente2", 0.2);
  // System.out.println("Aggregation score dopo il primo verdetto: " +
  // aggregationManager.read(aggid2).getReputation());
  //
  //
  // try {
  ...
  //
  // Collection<VerdictAggregation> coll =
  // db.getListOfAggregationByVerdict(v.getVerdictId());
  // if (coll.isEmpty()) {
  // System.out.println("No element found for verdict: " + v.getVerdictId() +
  // " producer= " + v.getJudgeId() + " consumer= "+ v.getChargedId());
  // } else {
  // System.out.println("Verdict: " + v.getVerdictId() +
  // " is part of the following aggregations ");
  // }
  ...
  // if (!verdictManager.updateAggregation(aggid1, "app1", "utente1", true,
  // "6utent2", 0.4)) {
  // System.out.println("updateAggregation: aggregation pointed by "+ aggid1 +
  // " does not exist");
  //
  // }
  ...
  // reputationManager.read("6utent2", "app1", false).getReputation());
  //
  // //System.out.println(new SimpleReputationEngine(new
  // UserApplicationConsumerFacade(wrapper),
  // // new UserApplicationProducerFacade(wrapper), new
  // VerdictFacade(wrapper)).readReputation("user2", "app1",
  // false).getReputation());
  // }
}
