...
public class CassandraQueryFacade {
   ...
   * @return
   */
  static CassandraQueryFacade getNewInstance(final CassandraWrapper w) {
    if (instance == null) {
   ...
   */
  void insertDbTableObject(final Object objToInsert) {
    try {
      ...
      for (final Field field : objToInsert.getClass().getDeclaredFields()) {
        ...
        if (field.getName().equals(primaryKeyFieldName)) {
          ...
          currentCol.setColumn(new Column(ByteBuffer.wrap(field.getName().getBytes()), pkByteBuffer, System
              .currentTimeMillis()));
        } else { // otherwise, we use the usual methodology
          currentCol.setColumn(new Column(ByteBuffer.wrap(field.getName().getBytes()), ByteBuffer.wrap(ClassType
              .objectToByteArray(type, field.get(objToInsert))), System.currentTimeMillis()));
        }
  ...
  void insertIntoList(final String superColumnFamilyName, final String keyOfList, final String cfKey) {
    try {
      ...
      if (listAttributes != null && listAttributes.getLength() == 1) {
        ...
        final Class<?> idClass = Class.forName(Constants.PRIMARY_KEY_PACKAGE + "." + columnFamilyName + "Id");
        final Method stringToObj = idClass.getMethod("parseString", new Class[] {String.class});
        final Method objToByteBuffer = idClass
            .getMethod("get" + columnFamilyName + "Id" + "AsByteBuffer", new Class[0]);
