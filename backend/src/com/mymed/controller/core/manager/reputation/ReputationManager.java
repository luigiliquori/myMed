...
public class ReputationManager extends AbstractManager implements
	...
	public ReputationManager(final StorageManager storageManager)
			throws InternalBackEndException {
		super(storageManager);
	}

	@Override
	public MReputationBean read(final String producerID,
			final String consumerID, final String applicationID)
			throws InternalBackEndException, IOBackEndException {
		final MReputationBean reputationBean = new MReputationBean();
		Map<byte[], byte[]> args = new HashMap<byte[], byte[]>();
		args = storageManager.selectAll(CF_REPUTATION, producerID
				+ applicationID);
		return (MReputationBean) introspection(reputationBean, args);
	}

	@Override
	public void update(final MInteractionBean interaction, final double feedback)
			throws InternalBackEndException, IOBackEndException {
		// REMINDER:
		// THE REPUTATION VALUE IS THE LAST GIVEN FEEDBACK

		storageManager.insertColumn(CF_REPUTATION, interaction.getProducer()
				+ interaction.getApplication(), "value", MConverter
				.doubleToByteBuffer(feedback).array());
	}
}
