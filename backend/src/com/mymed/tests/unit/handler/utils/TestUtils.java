...
/**
 * Class with utilities used to perform tests on the various requests handler on the backend.
 * <p>
 * In oder for these tests to work, it is necessary to have the Mymed backend code running within Glassfish. These tests
 * make the assumption that the backend is running on {@code localhost}, on port {@code 8080}.
 * 
...
public class TestUtils {
    ...
    /**
     * Perform a recursive validation on an JsonObject.
     * <p>
     * We keep this method here even if it is not used, in case we need it.
     * 
     * @param object
    ...
    private static boolean recursiveValid(final JsonObject object) {
        boolean returnValue = true;

        final Set<Entry<String, JsonElement>> jsonSet = object.entrySet();

        if (!jsonSet.isEmpty()) {
            final Iterator<Entry<String, JsonElement>> iter = jsonSet.iterator();

            while (iter.hasNext()) {
                final Entry<String, JsonElement> entry = iter.next();

                returnValue &= MJson.isValidElement(entry.getKey());

                if (entry.getValue().isJsonObject()) {
                    returnValue &= recursiveValid(entry.getValue().getAsJsonObject());
                }
            }
        }
    ...
    /**
     * Check that a 'user' JSON string is valid with regards to the 'user' JSON format as defined in {@link MUserJson}.
     * 
     * @param json
     *            the JSON string to be validated
     * @return true if is valid, false otherwise
    ...
    /**
     * Check that a 'search' JSON string is valid with regards to the 'search' JSON format as defined in
     * {@link MSearchJson}.
     * 
     * @param json
    ...
    public static String createAccessToken() {
        ...
        try {
            ...
            final byte[] mdBytes = digest.digest((MYMED_EMAIL + getFakePassword() + time).getBytes(Charset
                            .forName(CHAR_NAME)));
